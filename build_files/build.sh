...
# there's a bug with DistroShelf in that it may error when trying to uninstall, so i'm keeping it out of the above flatpak removal and attempting uninstall twice just in case
xargs -r flatpak remove -y --noninteractive app/com.ranfdev.DistroShelf/x86_64/stable
xargs -r flatpak remove -y --noninteractive app/com.ranfdev.DistroShelf/x86_64/stable

## this removes the gnome extension manager
